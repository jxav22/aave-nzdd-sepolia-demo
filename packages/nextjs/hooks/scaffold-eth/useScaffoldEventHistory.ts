...
>({
  ...
  const deployedOnBlock =
    ...
    typeof (deployedContractData as { deployedOnBlock?: unknown }).deployedOnBlock === "number"
      ? (deployedContractData as { deployedOnBlock: number }).deployedOnBlock
      : 0;

  const fromBlockValue = fromBlock !== undefined ? fromBlock : BigInt(deployedOnBlock);
