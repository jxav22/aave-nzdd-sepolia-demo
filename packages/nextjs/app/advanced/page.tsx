  ...
  return (
    <div className="flex flex-col items-center grow pt-8 pb-16 px-4">
      <div className="w-full max-w-2xl flex flex-col gap-6">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-[0.22em] opacity-60">Advanced</p>
          <h1 className="text-3xl font-bold mt-2">Protocol panel</h1>
          <p className="mt-2 text-sm opacity-70">
            Raw reserve state and unsequenced individual actions. The consumer flows are at{" "}
            <a href="/app" className="link">
              /app
            </a>
            ; this page exposes allowances, deposit receipt (aToken) and loan record (variable debt) balances directly.
          </p>
          <p className="mt-1 text-sm opacity-70">{config.marketId}</p>
        </div>
        ...
        {selectedAsset !== "dNZD" && (
          <div className="alert alert-success text-sm">
            After supplying {selectedAsset} as collateral, switch to the <strong>dNZD</strong> tab to borrow against it.
          </div>
        )}
